  ...
  <div class="page-container">
    ...
    <main class="container-fluid" v-show="!isLoading">
      <div class="viewer-container" ref="viewerContainer">
        ...
        <!-- 滾輪提示 -->
        <div v-if="showScrollHint && !selectedTargetData" class="scroll-hint">
          <span>滾動滑鼠滾輪移動視角</span>
        </div>
...
const initThreeScene = () => {
  ...
  camera.position.set(-0.42, 3.29, -10.85); // 更新初始攝影機位置
  ...
  controls.target.set(-0.42, -28, -46); // 更新初始目標點
  ...
  threeCanvas.value.addEventListener('wheel', onCanvasMouseWheel); // 新增滾輪事件監聽
};
  ...
  if (isAnimatingCamera) {
    ...
    if (camera.position.distanceTo(targetPosition) < 0.01) { // 檢查是否已到達目標
      ...
      if (selectedTargetData.value) {
        ...
        showScrollHint.value = false; // 進入座位視角時隱藏滾輪提示
        clearTimeout(scrollHintTimeout); // 清除滾輪提示的計時器
...
const onCanvasMouseWheel = (event) => {
  event.preventDefault(); // 防止瀏覽器滾動
  // 如果已經點擊座位，則禁用滾輪縮放
  if (selectedTargetData.value) return;
...
onUnmounted(() => {
  ...
  threeCanvas.value?.removeEventListener('wheel', onCanvasMouseWheel); // 移除滾輪事件監聽
